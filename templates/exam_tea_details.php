...
setcookie('e_id', $_GET['e_id'], time() + (86400 * 30), "/");
$query11 = "select e_title from exam where e_id='{$_GET['e_id']}'";
...
$query11 = "select u_id, fname, lname from takes_part natural join user where e_id='{$_GET['e_id']}' and sgrade is null";
$result11 = mysqli_query($con, $query11);
$n = 0;
if ($result11 && mysqli_num_rows($result11) > 0) {
    while ($vid = mysqli_fetch_assoc($result11)) {
        $uuid[$n] = $vid['u_id'];
        $nname[$n] = $vid['fname'] . ' ' . $vid['lname'];
        $n++;
    }
}
$query11 = "select count(u_id) as ccid from enroll where c_id='$cid'";
$result11 = mysqli_query($con, $query11);
$row = mysqli_fetch_assoc($result11);
$ccid = $row['ccid'];

$query11 = "select count(u_id) as bcid from takes_part where e_id='{$_GET['e_id']}'";
$result11 = mysqli_query($con, $query11);
$row = mysqli_fetch_assoc($result11);
$bcid = $row['bcid'];
...
<body>
    ...
    <div class="page-content p-5" id="content">
        ...
        <!-- Demo content -->
        <h3 class="display-4 text-dark"><?php echo $title; ?></h3>

        <!-- <button id="buttton_id"><a class="button"  href="#popup1">Let me Pop up</a></button> -->
        <hr>
        <div class="col-lg-12">

            <h4>Missing: <?php echo $ccid - $bcid; ?></h4>
            <div class="container">
                <!--image row start-->
                <div class="row1">
                    <!--image card start-->
                    <?php for ($i = 0; $i < $n; $i++) { ?>
                        <div class="image1" style="text-align:center;">
                            <!-- <img src="1.jpg" alt=""> -->
                            <div class="details1">
                                <h2><span><?php echo $nname[$i]; ?></span></h2>
                                <div class="more1">
                                    <a href="grading_tea.php?uuid=<?php echo $uuid[$i]; ?>" class="read-more1"><span>View </span></a>
                                </div>
                            </div>
                        </div>
                        <!--image card end-->
                    ...
                    <form method="post" enctype="multipart/form-data">
                        <div class="inp">
                            ...
                            <div class="boxx">
                                ...
                                <input type="text" name="etime" required></input>
                            </div>
